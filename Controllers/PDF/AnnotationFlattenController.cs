...
namespace EJ2MVCSampleBrowser.Controllers.PDF
{
    public partial class PdfController : Controller
    {
        ...
        public ActionResult AnnotationFlatten(string InsideBrowser, string submit, string checkboxFlatten, HttpPostedFileBase file)
        {
            if (submit == "Create Annotations")
            {
                ...
                FileStream videoStream = new FileStream(ResolveApplicationDataPath("Video.mp4"), FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite);
                //Create rich media content with video file
                PdfRichMediaContent videoContent = new PdfRichMediaContent("video", videoStream, "mp4");
                ...
                PdfBitmap image = new PdfBitmap(ResolveApplicationImagePath("richmedia.jpg"));
                ...
                FileStream audioStream = new FileStream(ResolveApplicationDataPath("Sound.mp3"), FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite);
                //Create rich media content with audio file
                PdfRichMediaContent soundContent = new PdfRichMediaContent("Sound", audioStream, "mp3");
                //Set the content type
                soundContent.ContentType = PdfRichMediaContentType.Sound;
                //Add content to the rich media
                richMediaAnnotationSound.Content = soundContent;

                PdfBitmap soundImage = new PdfBitmap(ResolveApplicationImagePath("richmedia_sound.jpg"));
                ...
                if (checkboxFlatten == "Flatten")
                {
                    ...
                    document.Close(true);
                    SourceStream.Position = 0;//Creates a new Loaded document.
        ...
        private void AddWatermarkAnnotation(PdfDocument document)
        {
            ...
            FileStream imageStream = new FileStream(ResolveApplicationImagePath("AdventureCycle.jpg"), FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
