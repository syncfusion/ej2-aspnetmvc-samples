...
namespace EJ2MVCSampleBrowser.Controllers.Grid
{
    public partial class GridController : Controller
    {
        ...
        public ActionResult LazyLoadGroupWithInfiniteScrolling()
        {
            if (customerDetails.Count == 0)
                BindCustomerDataSource();
            ViewData["lazyLoadData"] = customerDetails;
            return View();
        }
        private void BindCustomerDataSource()
        {
            ...
            for (int x = 0; x < 20000; x++)
            {
                ...
                customerDetails.Add(new CustomerDetails() { OrderID = OrderID + x, CustomerID = customerid[i], CustomerName = customername[j], CustomerAddress = customeraddress[k], ProductName = product[l], ProductID = x, Quantity = quantityperunit[m] });
                i++; j++; k++; l++; m++;
            }
        }
