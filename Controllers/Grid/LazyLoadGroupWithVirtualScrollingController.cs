...
namespace EJ2MVCSampleBrowser.Controllers.Grid
{
    ...
    {
        // GET: LazyLoadGroupWithInfiniteScrolling
        public static List<CustomerData> virtualCustomerDetails = new List<CustomerData>();
        public ActionResult LazyLoadGroupWithVirtualScrolling()
        {
            if (virtualCustomerDetails.Count == 0)
                BindData();
            ViewData["lazyLoadData"] = virtualCustomerDetails;
            return View();
        }
        private void BindData()
        {
            ...
            for (int x = 0; x < 20000; x++)
            {
                ...
                virtualCustomerDetails.Add(new CustomerData() { OrderID = OrderID + x, CustomerID = customerid[i], CustomerName = customername[j], CustomerAddress = customeraddress[k], ProductName = product[l], ProductID = x, Quantity = quantityperunit[m] });
                i++; j++; k++; l++; m++;
            }
        }
