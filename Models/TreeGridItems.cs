...
namespace EJ2MVCSampleBrowser.Models
{
    ...
    public class VirtualScrollData
    {
        ...
        public static List<VirtualScrollData> GetVirtualScrollData()
        {
            List<VirtualScrollData> DataCollection = new List<VirtualScrollData>();
        ...
        "Trace Collector", "Synthetic Tests Suite", "SIEM Platform"
            ...
            "Virtual Machine", "Database", "Storage", "Network", "Kubernetes", "Gateway", "Serverless", "Application", "Monitoring", "Security"
        };

            string[] regions = new string[] {
            "West US", "East US", "EU West", "EU Central", "Asia Pacific", "South America", "Canada Central", "Australia East"
        };

            string[] statuses = new string[] { "Running", "Stopped", "Degraded", "Maintenance" };
            string[] environments = new string[] { "Production", "Staging", "Development" };
            string[] priorities = new string[] { "Low", "High", "Critical" };
            ...
            {
                int parentId = taskId;
                string resourceType = resourceTypes[rand.Next(resourceTypes.Length)];
                string region = regions[p % regions.Length];
                string environment = environments[p % 3];
                string status = statuses[(p + 1) % statuses.Length];
                string priority = priorities[p % priorities.Length];
                string parentResourceId = resourceNames[rand.Next(resourceNames.Length)];
                DataCollection.Add(new VirtualScrollData
                {
                    TaskID = taskId,
                    ParentID = null,
                    ...
                    MonthlyCost = 500 + (p * 150),
                    Cpu = 45 + (p % 50),
                    Memory = 65 + (p % 30),
                    Disk = 72 + (p % 25),
                    NetworkMbps = 100 + (p * 50),
                    RequestsPerSec = 1000 + (p * 500),
                    ErrorRatePpm = 100 + (p * 20),
                    StorageGb = 500 + (p * 100),
                    InstanceCount = 2 + (p % 8),
                    UptimeDays = 180 + (p * 5),
                    TagsCount = 3 + (p % 7),
                    ComplianceScore = 75 + (p % 25),
                    Priority = priority,
                    SecurityFindings = p % 10
                });
                taskId++;
                int childrenPerParent = 25 + (p % 30);
                for (int c = 0; c < childrenPerParent; c++)
                {
                    string childStatus = statuses[(c + p) % statuses.Length];
                    string childEnvironment = environments[(c + p) % 3];
                    string childRegion = regions[(c + p + 2) % regions.Length];
                    string childresourceType = resourceTypes[rand.Next(resourceTypes.Length)];

                    
                    string childResourceId = resourceNames[rand.Next(resourceNames.Length)];
                    ...
                    DataCollection.Add(new VirtualScrollData
                    {
                        ...
                        MonthlyCost = 50 + (c * 5),
                        Cpu = rand.Next(10, 95),
                        Memory = rand.Next(15, 90),
                        Disk = rand.Next(20, 98),
                        NetworkMbps = 10 + (c % 100),
                        RequestsPerSec = 100 + (c * 50),
                        ErrorRatePpm = 10 + (c % 100),
                        StorageGb = 50 + (c * 10),
                        InstanceCount = 1,
                        UptimeDays = rand.Next(30, 365),
                        TagsCount = rand.Next(1, 15),
                        ComplianceScore = rand.Next(60, 100),
                        Priority = priorities[(c + p) % priorities.Length],
                        SecurityFindings = rand.Next(0, 20)
                    });
                    taskId++;
                }
            }
