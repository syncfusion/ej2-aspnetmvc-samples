    ...
    if (packageList) {
        for (var nuget of packageList) {
            ...
            if (!(/Syncfusion.EJ2.MVC5|Syncfusion.EJ2.GridExport.MVC5|Syncfusion.EJ2.Spreadsheet.AspNet.MVC5|Syncfusion.Licensing/.test(nugetName))) {
                var shellCode = shelljs.exec(`nuget list ${nugetName} -AllVersions -Source "http://nexus.syncfusion.com/repository/nuget-hosted/"`, { silent: true, async: false });
                if (shellCode.code === 0 && !shellCode.stdout.startsWith('No packages found.')) {
                    var versionLists = shellCode.stdout.match(new RegExp(`${nugetName} .*`, 'g')).map(output => { return output.replace(`${nugetName} `, '') });
                    ...
                    for (var versionList of versionLists) {
                        ...
                        var suffixVersion = (splitVersion[3] === undefined || null) ? '0' : splitVersion[3];
