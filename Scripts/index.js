...
function buildNotifications() {
    ...
    var compUpdates = data.filter(function(c){
        var t = (c.type || '').toString().toLowerCase();
        return t === 'new' || t === 'update';
    });
    var sampleUpdates = [];
    data.forEach(function(c) {
        var updates = (c.samples || []).filter(function(sample) {
        var t = (sample.type || '').toString().toLowerCase();
        return t === 'new' || t === 'update';
        });
        if (updates.length) {
        sampleUpdates.push({ name: c.name, directory: c.directory, samples: updates });
        }
    });
    var html = '';
